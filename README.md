...
### 📡 Мониторинг по требованию

**Site Monitor** теперь работает в режиме "по требованию":

- **🔄 Ручные проверки** - нажимайте кнопку "Проверить сейчас" для запуска проверки всех сайтов
- **⚡ Real-time уведомления** - получайте мгновенные уведомления через SSE при завершении проверок
- **💾 Сохранение ресурсов** - система не тратит ресурсы на постоянные проверки
- **📊 Актуальная статистика** - данные обновляются только при необходимости

#### 🔧 Как это работает:
1. **Запускайте проверки вручную** - кнопка "Проверить сейчас" запускает проверку всех активных сайтов
2. **Получайте уведомления** - SSE покажет результаты проверки в реальном времени
3. **Просматривайте результаты** - данные остаются актуальными до следующей проверки
4. **Настраивайте интервалы** - конфигурация интервалов применяется при ручном запуске
...
**Обратите внимание:** 
- Переменная `CHECK_INTERVAL` больше не используется для автоматических проверок
- Система работает в режиме "по требованию" - проверки запускаются только вручную
- Интервалы в конфигурации сайтов используются для задержек между проверками отдельных сайтов
